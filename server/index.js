...
app.use(cors());
const playersCollection = db_1.firestore.collection("players");
...
app.post("/rooms", (req, res) => {
    ...
    playersCollection
        ...
        .get()
        .then((doc) => {
        const roomRef = db_2.rtdb.ref("/rooms/" + (0, uuid_1.v4)());
...
app.post("/rooms/:rtdbRoomId", (req, res) => {
    ...
    const { nombre } = req.body;
    const { playerId } = req.body;
    const playersRef = db_2.rtdb.ref("/rooms/" + rtdbRoomId + "/players");
    playersRef.once("value", (snapshot) => {
        const players = snapshot.val();
        const playersList = (0, lodash_1.map)(players);
        if (playersList[1].nombre != "") {
            ...
            const player2Ref = db_2.rtdb.ref("/rooms/" + rtdbRoomId + "/players/1");
...
app.post("/change-data-player1/:rtdbRoomId", (req, res) => {
    const { rtdbRoomId } = req.params;
    const playersRef1 = db_2.rtdb.ref("/rooms/" + rtdbRoomId + "/players/0");
...
app.post("/change-data-player2/:rtdbRoomId", (req, res) => {
    const { rtdbRoomId } = req.params;
    const playersRef2 = db_2.rtdb.ref("/rooms/" + rtdbRoomId + "/players/1");
